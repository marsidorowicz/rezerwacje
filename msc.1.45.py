...
class ScrollingArea(Frame, object):
    ...
    def __init__(self, master, width=None, height=None, mousewheel_speed=2, scroll_horizontally=True, xscrollbar=None,
        ...
        super(ScrollingArea, self).__init__(master, **kw)
        ...
        for row in rows:
            ...
            if 'NAZWISKO' in row and x == 0:    # To display column names only ones
                for i in range(len(rows[0]) + 1):  # First row with column names
                    try:
                        self.lb[i].insert(END, rows[0][i])
                        x = 1 
                    except Exception as e:
                        pass
                    x = 1
            if 'NAZWISKO' not in row:
                if months[today.month - 1] in row:
                    for i in range(len(rows[0])+1):  # check for this month content and show it in listbox
                        try:
...
class GUI(Frame):
    ...
    def automated_listbox_creation(self):
        ...
        self.scrolling_area = ScrollingArea(self.Frame1)
